...
public sealed class BreakoutProgressionPageTests
{
    ...
    public void ProgressionPageUsesSavedHeatForGlitchUnlocks()
    {
        ...
        var blacklightBricks = view.Cards.First(card => card.Title == "Blacklight Bricks");
        ...
        Assert.That(blacklightBricks.UnlockHint, Does.Contain("Heat 21"));
    ...
    public void UnlockLadderTypeFilterShowsGlitches()
    {
        ...
        Assert.That(epicGlitchView.Cards.Any(card => card.Title == "Magnet Storm"), Is.True);
    }
    ...
    public void AuthoredLiveUnlocksUseParallelDropAndGlitchHeatTracks()
    {
        ...
        Assert.That(capsuleRoulette.Family, Does.Contain("Uncommon"));
        Assert.That(driftRows.UnlockHint, Does.Contain("Heat 10"));
        Assert.That(driftRows.Description, Does.Contain("opposite directions"));
        Assert.That(driftRows.Family, Does.Contain("Uncommon"));
        Assert.That(hotCorners.UnlockHint, Does.Contain("Heat 11"));
        Assert.That(hotCorners.Description, Does.Contain("kick balls back toward center"));
        Assert.That(hotCorners.Family, Does.Contain("Uncommon"));
        Assert.That(flickerBricks.UnlockHint, Does.Contain("Heat 12"));
        ...
        Assert.That(splitHorizon.Family, Does.Contain("Uncommon"));
        Assert.That(brickConveyor.UnlockHint, Does.Contain("Heat 17"));
        Assert.That(brickConveyor.Description, Does.Contain("crawl sideways"));
        Assert.That(brickConveyor.Family, Does.Contain("Rare"));
        Assert.That(pickupPinball.UnlockHint, Does.Contain("Heat 19"));
        Assert.That(pickupPinball.Description, Does.Contain("bounce off walls and bricks"));
        Assert.That(pickupPinball.Family, Does.Contain("Uncommon"));
        Assert.That(magnetStorm.UnlockHint, Does.Contain("Heat 20"));
        Assert.That(magnetStorm.Description, Does.Contain("tug balls plus capsules"));
        Assert.That(magnetStorm.Family, Does.Contain("Epic"));
        Assert.That(blacklightBricks.UnlockHint, Does.Contain("Heat 21"));
        Assert.That(blacklightBricks.Description, Does.Contain("hide until first contact"));
        Assert.That(blacklightBricks.Family, Does.Contain("Rare"));
        Assert.That(rewindWall.UnlockHint, Does.Contain("Heat 22"));
        Assert.That(rewindWall.Description, Does.Contain("rebuilds once"));
        Assert.That(rewindWall.Family, Does.Contain("Epic"));
        Assert.That(scoreLeak.UnlockHint, Does.Contain("Heat 23"));
        Assert.That(scoreLeak.Description, Does.Contain("Score drains between brick breaks"));
        Assert.That(scoreLeak.Family, Does.Contain("Rare"));
        ...
        Assert.That(prismShuffle.Family, Does.Contain("Rare"));
        Assert.That(cloneStatic.UnlockHint, Does.Contain("Heat 27"));
        ...
        Assert.That(staticJackpot.Family, Does.Contain("Rare"));
        ...
        Assert.That(capsuleBlackout.UnlockHint, Does.Contain("Heat 36"));
        Assert.That(capsuleBlackout.Description, Does.Contain("hides the next wave"));
        Assert.That(capsuleBlackout.Family, Does.Contain("Uncommon"));
        Assert.That(brickquake.UnlockHint, Does.Contain("Heat 37"));
        ...
        Assert.That(turboTax.Family, Does.Contain("Rare"));
        Assert.That(warpJam.UnlockHint, Does.Contain("Heat 39"));
        Assert.That(warpJam.Description, Does.Contain("wrong linked exit"));
        Assert.That(warpJam.Family, Does.Contain("Epic"));
        Assert.That(neonFlood.UnlockHint, Does.Contain("Heat 40"));
        Assert.That(neonFlood.Description, Does.Contain("combo spikes"));
        Assert.That(neonFlood.Family, Does.Contain("Rare"));
        Assert.That(meltdownCore.UnlockHint, Does.Contain("Heat 44"));
